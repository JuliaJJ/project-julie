<script>
    ...
    $: songLength = spotify ? spotify.timestamps.end - spotify.timestamps.start : 0;

    $: if (spotify) {
        const elapsed = (Date.now() - spotify.timestamps.start) / songLength;
        progress.set(elapsed, { duration: 0 });
        progress.set(1, {
            duration: songLength - songLength * elapsed
        });
    }
</script>
...
{#if spotify !== null}
   {#if $lanyard}
      <div class="flex bg-sky-50 shadow-lg rounded-lg mx-4 md:mx-auto max-w-md md:max-w-2xl mb-4 relative hover:bg-white">
         <a href="https://open.spotify.com/track/{$lanyard.spotify.track_id}" class="inline-block ml-4 mb-2 w-6 absolute top-5 right-5 hover:fill-sky-900"><img src={spotifyIcon.src} alt="Spotify" class="hover:fill-sky-900"></a>
         <div class="flex w-full items-start p-5">
            ...
            <div class="w-full">
               ...
               <div class="flex flex-col w-full bg-slate-300 rounded-full h-2.5 dark:bg-slate-300">
                  <div class="bg-sky-900 h-2.5 rounded-full" style:width="{$progress * 100}%"></div> 
               </div>
            </div>
         </div>
      </div>
